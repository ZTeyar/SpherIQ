#!/usr/bin/env python3
"""Variant: Face embeddings only — early fusion cubemap with discrete grid pos emb + face-ID, no 3D spherical RoPE."""
...
def train_face_only(train_loader, val_loader, num_epochs=40, device='cuda',
                    is_pretrained=True, save_prefix='faceonly', continue_from=0):
...
def main():
    ...
    prefix = args.save_prefix or f'faceonly_{args.dataset}_fold{args.fold}'
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    display_text(f"Device={device}  Variant=FaceOnly  Dataset={args.dataset}  Fold={args.fold}")
    ...
    train_face_only(tl, vl, num_epochs=args.epochs, device=device,
