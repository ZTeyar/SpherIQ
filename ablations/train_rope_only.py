#!/usr/bin/env python3
"""Variant: 3D RoPE only — early fusion cubemap with 3D spherical RoPE, no explicit face-ID embeddings in encoder."""
...
def train_rope_only(train_loader, val_loader, num_epochs=40, device='cuda',
                    is_pretrained=True, save_prefix='ropeonly', continue_from=0):
...
def main():
    ...
    prefix = args.save_prefix or f'ropeonly_{args.dataset}_fold{args.fold}'
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    display_text(f"Device={device}  Variant=RoPEOnly  Dataset={args.dataset}  Fold={args.fold}")
    ...
    train_rope_only(tl, vl, num_epochs=args.epochs, device=device,
