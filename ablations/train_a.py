#!/usr/bin/env python3
"""Variant A: vanilla pyiqa MUSIQ on ERP directly. Uses unified train_model()."""
...
from common import make_variant_a, load_ab_data, train_model, seed_everything, display_text


def train_a(train_loader, val_loader, num_epochs=40, device='cuda',
            is_pretrained=True, save_prefix='variantA', continue_from=0):
    bml = 2e-5 if is_pretrained else 1e-4
    mml = 1e-4 if is_pretrained else 4e-4

    model = make_variant_a(pretrained=is_pretrained).to(device)
    m = model.model
...
def main():
    ...
    prefix = args.save_prefix or f'variantA_{args.dataset}_fold{args.fold}'
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    is_pretrained = not args.no_pretrained
    display_text(f"Device={device}  Variant=A  Dataset={args.dataset}  Fold={args.fold}  "
                 f"Pretrained={is_pretrained}")

    tl, vl = load_ab_data('A', args.dataset, data_dir=args.data_dir,
                           num_folds=args.num_folds, fold_index=args.fold,
                           batch_size=args.batch_size, cpu_workers=args.cpu_workers,
                           seed=args.seed)
    train_a(tl, vl, num_epochs=args.epochs, device=device,
            is_pretrained=is_pretrained, save_prefix=prefix,
